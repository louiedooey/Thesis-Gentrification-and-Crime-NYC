...
bkg_ofn_df <- bkg_ofn %>% as.data.frame()
...
# 
# ggplot() + 
#   geom_line(data = top_10_crime, aes(x -))
...
# writeOGR(obj = ct_map_no_gentri_spa, dsn = "ct_map_no_gentri", layer = "layers", driver = "ESRI Shapefile")
beep()


ct_poly <- readShapePoly("ct_map_no_gentri/layers.shp")
...
View(ct_map_no_gentri)

names(ct_poly)

fit_ct <- lm(FEL_CT_CHANGE ~ CT_GENTRI_SCORE, data = ct_poly)

names(ct_map_no_gentri)
...
lmoran_f <- localmoran(map_analysis$FEL_CT_CHANGE, list_w)
lmoran_g <- localmoran(map_analysis$CT_GENTRI_SCORE, list_w)
summary(lmoran_f)
summary(lmoran_g)
# padronize the variable and save it to a new column 
map_analysis$s_fel <- scale(map_analysis$FEL_CT_CHANGE) %>% as.vector()
map_analysis$s_gentri <- scale(map_analysis$CT_GENTRI_SCORE) %>% as.vector()
...
x <- map_analysis$s_fel
y <- map_analysis$lag_s_fel %>% as.vector()
xx <- data.frame(x, y)
...
ggsave("visualizations/lisa-ct-fel-no-gentri.svg", lisa_fel_ct_plot, width = 8, height = 5)
ggsave("visualizations/lisa-grid-no-gentri.svg", lisa_grid, width = 14, height = 10)
...
fit_1 <- stan_glm(FEL_NTA_CHANGE ~ NTA_GENTRI_SCORE, data = nta_map_no_gentri_2, refresh = 0)

fit_2 <- stan_glm(FEL_CT_CHANGE ~ CT_GENTRI_SCORE, data = nta_map_no_gentri_2, refresh = 0)

p <- print(fit_1, digits = 5)
